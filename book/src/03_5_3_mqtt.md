...
✅ Verify your subscription is working by logging the received topic and running the `example_client` in parallel in it's own terminal. You should receive a board LED command roughly every second.
...
```rust
    ...
    match message.details() {
        Complete(token) => {
            ...
            // there is an impl block to convert a slice to a color in lib, use it:
...
- Implement the same task but by subscribing to all "command" messages, combining `cmd_topic_fragment(uuid)` with a trailing `#` wildcard to practice hierarchy in mqtt.
